...
const tikCompleted=matrix([{ 'Order ID':'T-DONE','Order Status':'Concluído','SKU ID':'SKU-DONE','Seller SKU':'GA903','Product Name':'Concluido','Quantity':'1','SKU Subtotal Before Discount':'BRL 100,00','SKU Seller Discount':'BRL 0','Created Time':'06/30/2026 10:18:10 AM','Fulfillment Type':'Fulfillment by seller' }]);
result=transformTikTok({...tikCompleted,channelName:'Click Piscinas',taxRate:8,commissionBelowRate:10,commissionBelowFixed:4,commissionAboveRate:6,commissionAboveFixed:6,freightRate:4},db);
assert.equal(result.rows[0][6],'Venda','Pedido concluido deve ser venda');assert(Math.abs(result.rows[0][16]+12)<1e-9);assert(Math.abs(result.rows[0][17]+4)<1e-9);assert.equal(result.rows[0][18],0);assert(Math.abs(result.rows[0][19]-84)<1e-9);
const tikUnknownPaid=matrix([{ 'Order ID':'T-UNKNOWN','Order Status':'Delivered','SKU ID':'SKU-U','Seller SKU':'GA903','Product Name':'Entregue','Quantity':'1','SKU Subtotal Before Discount':'BRL 40,00','SKU Seller Discount':'BRL 0','Created Time':'06/30/2026 10:18:10 AM','Fulfillment Type':'Fulfillment by seller' }]);
result=transformTikTok({...tikUnknownPaid,channelName:'Click Piscinas',taxRate:8},db);
assert.equal(result.rows[0][6],'Venda','Status positivo ou desconhecido com faturamento nao pode ser cancelado');assert(Math.abs(result.rows[0][16]+8)<1e-9);assert(Math.abs(result.rows[0][17]+1.6)<1e-9);
