...
function dateOnly(value, monthFirst) {
  const raw = text(value);
  let match = raw.match(/^(\d{4})-(\d{2})-(\d{2})/);
  if (match) return `${match[1]}-${match[2]}-${match[3]}`;
  match = raw.match(/^(\d{2})\/(\d{2})\/(\d{4})/);
  if (match) return monthFirst ? `${match[3]}-${match[1]}-${match[2]}` : `${match[3]}-${match[2]}-${match[1]}`;
...
function classifyTikTokStatus(rawStatus, revenue) {
  if (!revenue) return 'Amostra Grátis';
  const status = norm(rawStatus);
  if (status.includes('cancel')) return 'Cancelado';
  if (status.includes('devol') || status.includes('return') || status.includes('refund')) return 'Devolução';
  if (status.includes('nao pago') || status.includes('unpaid') || status.includes('payment failed')) return 'Não pago';
  return 'Venda';
}
function transformTikTok(payload, pricingDatabase) {
  ...
  const commissionBelowRate = Number.isFinite(Number(payload.commissionBelowRate)) ? Number(payload.commissionBelowRate) / 100 : 0.10;
  const commissionBelowFixed = Number.isFinite(Number(payload.commissionBelowFixed)) ? Number(payload.commissionBelowFixed) : 4;
  const commissionAboveRate = Number.isFinite(Number(payload.commissionAboveRate)) ? Number(payload.commissionAboveRate) / 100 : 0.06;
  const commissionAboveFixed = Number.isFinite(Number(payload.commissionAboveFixed)) ? Number(payload.commissionAboveFixed) : 6;
  const freightRate = Number.isFinite(Number(payload.freightRate)) ? Number(payload.freightRate) / 100 : 0.04;
  if ([commissionBelowRate, commissionAboveRate, freightRate].some((rate) => rate < 0 || rate > 1) || [commissionBelowFixed, commissionAboveFixed].some((amount) => amount < 0)) throw new Error('Configuracao de comissao ou frete do TikTok invalida.');
  const output = source.filter((row) => text(value(row, 'Order ID')) && !['Platform unique order ID.', 'x'].includes(text(value(row, 'Order ID')))).map((row) => {
    ...
    const revenue = subtotal - sellerDiscount, date = dateOnly(value(row, 'Created Time'), true);
    if (!date) throw new Error(`Data inválida em 'Created Time', linha ${row._line}.`);
    const below50 = revenue < 50;
    let commission = revenue ? -(revenue * (below50 ? commissionBelowRate : commissionAboveRate) + (below50 ? commissionBelowFixed : commissionAboveFixed)) : 0;
    let freight = revenue ? -(revenue * freightRate) : 0;
    const orderStatus = text(value(row, 'Order Status'));
    const status = classifyTikTokStatus(orderStatus, revenue);
    const cancelled=status==='Cancelado'||status==='Não pago'||status==='Devolução';
...
function transformMagalu(payload, pricingDatabase, idMap) {
  ...
  const sold=new Set(['Pagamento aprovado','Nota fiscal gerada e aprovada','Pedido despachado','Pedido entregue'].map(norm));
  const output=items.map((item)=>{
    const row=item.row,m=item.money,s=summaries.get(item.order),pack=packageMap.get(item.order)||{};
    const statusRaw=text(value(pack,'Status pacote no momento que o relatório foi solicitado')),status=sold.has(norm(statusRaw).replace(/\s+/g,' '))?'Venda':'Cancelado',cancelled=status==='Cancelado';
